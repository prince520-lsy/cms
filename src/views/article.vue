<template>
    <div>
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/' }">page backs</el-breadcrumb-item>
            <el-breadcrumb-item>content manage</el-breadcrumb-item>
        </el-breadcrumb>
        <el-card>
            <template v-slot:header>
                <el-row type="flex" justify="space-between">
                    <div>共 {{total}} 条记录</div>
                    <el-button round type="primary" size="small" @click="addArticle">+ Add Pages</el-button>
                </el-row>
            </template>
            ...
            <span>
                <el-form ref="form" :model="article" :rules="rules" label-width="80px">
                    <el-form-item label="title" prop="stem">
                        <el-input v-model="article.stem" auto-complete="on"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item>
                        <el-button type="primary" size="small" @click="submit">Yes</el-button>
                        <el-button size="small" @click="close">No</el-button>
                    </el-form-item>
                </el-form>

            </span>
        </el-drawer>
        <el-drawer title="page preview" :visible.sync="previewShow">
...
<script>
import { articleList, addArticle, delArticle, getAricledetails, updateArticle } from '@/api/article'
...
export default {
  ...
  methods: {
    ...
    del (id) {
      console.log(167, id)
      this.$confirm('Comfirm to delete this page?', '提示', {
    ...
    async edit (id) {
      ...
      const res = await getAricledetails({ id })
      console.log(170, res)
      this.article = res.data.data
    },
    async preview (id) {
      this.previewShow = true
      const res = await getAricledetails({
        id
      })
      this.article = res.data.data
    }
